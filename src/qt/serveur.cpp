...
void Serveur::errorSocket(QAbstractSocket::SocketError error)
{
	switch(error)
	{
		case QAbstractSocket::HostNotFoundError:
            affichage->append(tr("<em>ERROR : can't find stakerush server.</em>"));
			break;
...
{
    affichage->append("Connecting... to stakerush IRC. Please wait.");

	sendData("USER "+pseudo+" localhost "+serveur+" :"+pseudo);
    sendData("NICK "+pseudo);
    affichage->append("Connected to stakerush IRC.");
